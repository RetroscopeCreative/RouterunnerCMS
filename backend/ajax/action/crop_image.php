	...
	foreach ($post["value"] as $field => $src) {
		...
		if (isset($value["src"])) {
			...
			*/
			...
			if (isset($field_data["max-width"]) || isset($field_data["max-height"])) {
				if ($layer->getWidth() > $layer->getHeight()) {
					if (isset($field_data["max-width"]) && intval($field_data["max-width"]) < $layer->getWidth()) {
						$layer->resizeInPixel(intval($field_data["max-width"]), null, true);
					}
					if (isset($field_data["max-height"]) && intval($field_data["max-height"]) < $layer->getHeight()) {
						$layer->resizeInPixel(null, intval($field_data["max-height"]), true);
					}
				} else {
					if (isset($field_data["max-height"]) && intval($field_data["max-height"]) < $layer->getHeight()) {
						$layer->resizeInPixel(null, intval($field_data["max-height"]), true);
