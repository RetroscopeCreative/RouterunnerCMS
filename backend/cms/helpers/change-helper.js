...
helper.change = $.extend({}, routerunner.components.helper, {
    ...
    log_change: function(change_obj, fn) {
        var url = routerunner.settings["BACKEND_DIR"] + "/backend/ajax/action/log_change.php";
        var data = {
    ...
    apply_change: function(change_obj, fn) {
        ...
        if (change_obj.section == "routerunner-page-properties") {
            url = routerunner.settings["BACKEND_DIR"] + '/backend/ajax/action/apply_pageprops.php';
            data.resource = routerunner.page.pageproperties.resource;
        } else {
            url = routerunner.settings["BACKEND_DIR"] + '/backend/ajax/action/apply_change.php';
    ...
    delete_change: function(change_obj) {
        var success = false;
        var url = routerunner.settings["BACKEND_DIR"] + '/backend/ajax/action/delete_change.php';
        var data = {
            change_id: change_obj.id
        };
        var params = {
