...
if ($succeed = \Routerunner\Form::submit($runner->form, $errors, $return_SQL, $return_params)) {
	...
	if (strpos($return_SQL, "INSERT") === 0) {
		$return_SQL = str_replace(
			array("`nonce`, ", "`id`, ", ", `usergroup`", ", `pwd_confirm`"),
			"", $return_SQL);
		$return_SQL = str_replace(array(":nonce, ", ":id, ", ", :usergroup", ", :pwd_confirm"),
			"", $return_SQL);
		if (!$pwd_change) {
		...
		$return_SQL = str_replace(
			array("`nonce` = :nonce, ", ", `usergroup` = :usergroup",
				", `pwd_confirm` = :pwd_confirm"), "", $return_SQL);
		if (!$pwd_change) {
			$return_SQL = str_replace(", `pwd` = :pwd", "", $return_SQL);
		}
	}
	unset($return_params[":nonce"], $return_params[":usergroup"],
	...
	} elseif (empty($return_params[":licence"])) {
		$return_params[":licence"] = null;
	}
